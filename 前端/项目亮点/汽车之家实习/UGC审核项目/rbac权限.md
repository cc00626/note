...
通过前端路由表和后端动态路由进行配置，前端维护一个完成的routerMap，包含所有路由的信息，用户登录成功后，后端返回角色的path数组，我编写了一个递归函数。

对后端的路由表进行深度遍历，当子路由具备权限的时候，父节点进行显示。否则将组件替换为NoAcess的组件。

同时封装了AuthRoute高阶组件，动态处理token和未登录跳转。



### 按钮的权限如何实现

通过封装自定义hooks，登录的时候会返回一个权限列表，hook接受一个code，返回一个布尔类型的值，在组件内直接通过组建的disabled 或者三元表达式控制组建的显示和渲染。
